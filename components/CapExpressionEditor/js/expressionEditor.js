	...
	protonode.getOperator = function () {
		...
		for(i=0,l=grammar.operators.length; i<l; i+=1) {
			...
			if(op.symbol === merged.symbol && m === this.operands.length) {
				//Adding this check to exactly match the operands in grammar and the expression's operands
				//Also adding to allow cross type additions/subtraction and other arithmetic operations.
				//Author:SuSi
				var allOpTypeMatches=false;
				for(j=0; j<m; j+=1) {
					t = this.operands[j].getType();
					if(t !== null && !t.is(this.getType(op.operandTypes[j]),true)) 
					{
						allOpTypeMatches=false;
						break;
					}
						
					else
						allOpTypeMatches=true;
				}
				if(allOpTypeMatches === true) {
