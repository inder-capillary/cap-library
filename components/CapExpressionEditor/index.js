...
class CapExpressionEditor extends React.Component {
  ...
  componentDidMount() {
     ...
     */
    const { grammar, expected, typeSource, delims, expression } = this.props;
    this.$expressionEditorContainer = $(findDOMNode(this.expressionEditorContainer));
  ...
  render() {
    const {className, onChange, grammar, expectedTypes, typeSource, delims, ...rest} = this.props;
